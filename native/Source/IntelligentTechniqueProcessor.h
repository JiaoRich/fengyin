...
public:
    ...
    void noteStarted(double nowMs) noexcept
    {
        ...
        for (auto& state : states) state.aboveSinceMs = 0.0;
    }
...
private:
    ...
    [[nodiscard]] static Curve curveFor(PerformanceTechnique technique, float strength) noexcept
    {
        // “自然”强度接近参考曲线：输入约 113..127，输出最多约 72%。
        ...
        return { std::clamp(0.89f - thresholdShift, 0.68f, 0.96f),
                 std::clamp(0.80f - thresholdShift * 0.65f, 0.58f, 0.90f),
                 std::min(1.0f, 0.42f + strength * 0.60f),
                 110.0, 120.0, 120.0f, 190.0f };
    }
